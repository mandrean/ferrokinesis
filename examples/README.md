...
`Cargo.lock` (Rust) and `go.sum` (Go) are committed for reproducible builds.
Node.js users should run `npm install` inside `examples/node/` to generate `package-lock.json` locally.
